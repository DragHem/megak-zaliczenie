        ...
        <div className={"overflow-y-auto grid place-items-center p-10 scrollbar-hide"}>
            ...
            <div className="overflow-x-scroll w-full scrollbar-hide">
                <table className="table w-full">
                    ...
                    <tbody>
                    {state.data.product.map(x=><tr className={"hover"}><td >{x.name}</td><td>{x.price}</td><td>{x.userIDs.map(user=><p>{state.data.users.find(users=>users.id==user)?.name}</p>)}</td><td>{Math.floor(x.price/x.userIDs.length*100)/100}</td></tr>)}
                    </tbody>
                </table>
            </div>
            <Divider/>
            <table className="table w-full">
                ...
                <tbody>
                {state.data.users.map(x=><tr className={"hover"}><td>{x.name} </td><td>{Math.floor(state.data.product.reduce((acc,curr)=>curr.userIDs.includes(x.id)?acc+curr.price/curr.userIDs.length:acc,0)*100)/100}</td></tr>)}

                </tbody>
            </table>

            <Button primary onClick={()=>handleSubmit()}>Zatwierdź zrzutkę</Button>
        </div>
    )
}
  ...
  return (
    <>
      ...
      <p className={"text-lg text-center"}>
        {kittySwitch ? "Nie aktywne" : "Aktywne"} zrzutki{" "}
        <input
          type="checkbox"
          className="toggle rotate-1 toggle-warning"
          onChange={() => setKittySwitch((prevState) => !prevState)}
          checked={!kittySwitch}
        />
      </p>
      {kitties &&
        kitties.map((kitty) => (
          <KittyCard isEnded={kittySwitch} kitty={kitty} key={kitty.id} />
        ))}
    </>
  );
};
